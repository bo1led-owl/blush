...
#define ARRAY_LIST(T, NAME)                                            \
    ...
    T NAME##_getOr(const NAME* arr, size_t index, T default_value);    \
...
#define ARRAY_LIST_IMPL(T, NAME)                                        \
    ...
    void NAME##_moveElement(NAME* arr, size_t from, size_t to) {        \
        ...
        T elem = NAME##_get(arr, from);                                 \
        if (from < to) {                                                \
            memmove(arr->items + from, arr->items + from + 1,           \
                    sizeof(T) * (to - from));                           \
            arr->items[to] = elem;                                      \
        } else {                                                        \
            memmove(arr->items + to + 1, arr->items + to,               \
                    sizeof(T) * (from - to));                           \
            arr->items[to] = elem;                                      \
        }                                                               \
    }                                                                   \
